...
public class RobotHelper {
    ...
    public static void initializePIDController(Robot robot) {
        // this.pidController.setInputRange((double)
        // context.getSimulationInput()
        // .getParameterInt(MIN_DIST_SOLDIER_ROBOT), (double) context
        // .getSimulationInput().getParameterInt(MAX_DIST_SOLDIER_ROBOT));
        robot.getPidController().setOutputRange(robot.getMinimumSpeed(),
                robot.getMaximumSpeed());
        // this.pidController.setSetpoint((double) context.getSimulationInput()
        // .getParameterInt(MIN_DIST_SOLDIER_ROBOT));
        robot.getPidController().setSetpoint(0);
        robot.getPidController().enable();
    }

    /**
     * Quadrant Load balancing algorithm
     */
    public static Location quadrantLoadBalancing(RctaContext context,
        ...
        for (Human civilian : soldiers) {
            if (civilian.getName().contains("CIV")) {
                if (RctaEnvironmentHelper.isVisible(context, vipLoc,
                        civilian.getLocation(), 0)) {
                    double deltaX =
                            civilian.getLocation().getX() - vipLoc.getX();
                    double deltaY =
                            vipLoc.getY() - civilian.getLocation().getY();
                    double angleBetween = RctaEnvironmentHelper.getAngle(
     ...
     */
    public static Location calculateCiviliansVectors(RctaContext context,
        ...
        for (Human civilian : soldiers) {
            if (civilian.getName().contains("CIV")) {
                if (RctaEnvironmentHelper.isVisible(context, vipLoc,
                        civilian.getLocation(), 0)) {
                    double angleBetween = RctaEnvironmentHelper.getAngle(vipLoc,
     ...
     */
    public static Location findNearestLocation(RctaContext context,
        Location targetLocation, Location desiredLocation,
        double minimumDistanceFromVIP) {
        double desiredAngleFromVIP = RctaEnvironmentHelper
        ...
        while (counter < 180) {
            ...
            if (RctaEnvironmentHelper.isAccessible(
            ...
            } else if (RctaEnvironmentHelper.isAccessible(
                    context.getEnvironmentModel(),
                    new MapLocationAccessibility(),
    ...
    public static Location findNewLocation(Robot robot, RctaContext context, Location currentLocation,
        Location intendedLocation) {
        double movementAngle = RctaEnvironmentHelper.getAngle(currentLocation,
