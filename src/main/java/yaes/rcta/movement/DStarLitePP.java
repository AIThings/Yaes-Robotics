...
public class DStarLitePP extends DStarLite<Location> implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5793307947544490717L;
	EnvironmentModel _map;

	/**
	 * Instantiates D* search over the given map.
	 * 
	 * @param map
	 * @param start
	 * @param goal
	...
	protected Collection<Location> nbrs(Location s) {
		...
		for (int x = -1; x <= 1; x++) {
			for (int y = -1; y <= 1; y++) {
				...
				if (loc.getX() > _map.getXHigh() || loc.getX() < _map.getXLow() || loc.getY() > _map.getYHigh()
	...
	protected boolean isAccessible(Location loc) {
		...
		return mapAccess.isAccessible(_map, loc);
	}
