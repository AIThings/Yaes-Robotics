...
public class Context extends AbstractContext implements constRCTA, Serializable {
	...
	public void initialize(SimulationInput sip, SimulationOutput sop) {
		...
		VIP = AgentBuilder.createSoldier(this, "VIP",
				new Location(sip.getParameterInt(START_X),
						sip.getParameterInt(START_Y) + sip.getParameterInt(Y_OFFSET)),
				new Location(sip.getParameterInt(DESTINATION_X),
						sip.getParameterInt(DESTINATION_Y) + sip.getParameterInt(Y_OFFSET)),
				2.0);
		humans.add(VIP);
		for (Human hm : humans) {
